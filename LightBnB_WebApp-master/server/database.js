...
const getUserWithEmail = function(email) {
  ...
  return pool
    ...
    .then((result) => {
      ...
      //if user with that email exist return user object
      if (result) {
...
const getUserWithId = function(id) {
  return pool
    ...
    .then((result) => {
      console.log(result.rows);
      //if user with that id exist return user object
      if (result) {
        return result.rows[0];
      } else {
        //else return null
...
const getAllProperties = (options, limit = 10) => {
  ...
  if (options.minimum_price_per_night && options.maximum_price_per_night) {
    ...
    if (queryParams.length === 2) {
      queryString += `WHERE cost_per_night >=  ($${queryParams.length - 1}) * 100 `;
  ...
  `;
  console.log('end of query string', queryString);
  return pool.query(
...
const addProperty = function(property) {
  ...
  return pool
    ...
    .then((result) => {
      console.log('results.row on success!',result.rows)
      return result.rows[0];
    })
    .catch((err) => {
      console.log('results.row hereerererererereHERHERHERHEHRFK', results.rows);
      console.log('addUser', err.message);
    });
